...
abstract class Calendar implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\CalendarTableMap';
     ...
     */
    protected int|null $calendar_id = null;
     ...
     */
    protected string|null $foregroundcolor = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\Calendar object.
     */
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : CalendarTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->calendar_id = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : CalendarTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->name = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : CalendarTableMap::translateFieldName('AccessToken', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->accesstoken = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : CalendarTableMap::translateFieldName('BackgroundColor', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->backgroundcolor = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : CalendarTableMap::translateFieldName('ForegroundColor', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->foregroundcolor = $columnValue !== null ? (string)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\Calendar object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildCalendarQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
    ...
    public function getByPosition(int $pos)
    {
        return match ($pos) {
            0 => $this->getId(),
            1 => $this->getName(),
            2 => $this->getAccessToken(),
            3 => $this->getBackgroundColor(),
            4 => $this->getForegroundColor(),
            default => null
        };
    }
    ...
    {
        $tableMap = CalendarTableMap::getTableMap();
        $criteria = new Criteria(CalendarTableMap::DATABASE_NAME);

        if ($this->isColumnModified(CalendarTableMap::COL_CALENDAR_ID)) {
            $criteria->setUpdateValue($tableMap->getColumn('calendar_id'), $this->calendar_id);
        }
        if ($this->isColumnModified(CalendarTableMap::COL_NAME)) {
            $criteria->setUpdateValue($tableMap->getColumn('name'), $this->name);
        }
        if ($this->isColumnModified(CalendarTableMap::COL_ACCESSTOKEN)) {
            $criteria->setUpdateValue($tableMap->getColumn('accesstoken'), $this->accesstoken);
        }
        if ($this->isColumnModified(CalendarTableMap::COL_BACKGROUNDCOLOR)) {
            $criteria->setUpdateValue($tableMap->getColumn('backgroundColor'), $this->backgroundcolor);
        }
        if ($this->isColumnModified(CalendarTableMap::COL_FOREGROUNDCOLOR)) {
            $criteria->setUpdateValue($tableMap->getColumn('foregroundColor'), $this->foregroundcolor);
        }
    ...
    {
        $tableMap = CalendarTableMap::getTableMap();
        $query = ChildCalendarQuery::create();
        $calendar_idColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('calendar_id'));
        $query->addAnd($calendar_idColumn, $this->calendar_id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\Calendar (or compatible) type.
    ...
    /**
     * Gets calendars objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this Calendar is new, it will return
    ...
    /**
     * Sets a collection of calendars objects related by a one-to-many relationship
    ...
    /**
     * Returns the number of related calendars objects.
     *
     * @param \Propel\Runtime\ActiveQuery\Criteria|null $criteria
     * @param bool $distinct
     * @param \Propel\Runtime\Connection\ConnectionInterface|null $con
     *
     * @return int Count of related calendars objects.
     */
     ...
     * Otherwise if this calendars is new, it will return
     * an empty collection; or if this calendars has previously
     ...
     * @return \ChurchCRM\model\ChurchCRM\Base\Collection\CalendarEventCollection
     */
    public function getCalendarEventsJoinEvent(
    ...
    /**
     * Associate a Event with this object through the calendar_events cross reference table.
     *
    ...
    /**
     * Remove event of this object through the calendar_events cross reference table.
     *
     * @param \ChurchCRM\model\ChurchCRM\Base\Event $event
     *
     * @return static
