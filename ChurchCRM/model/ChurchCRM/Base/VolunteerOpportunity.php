...
abstract class VolunteerOpportunity implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\VolunteerOpportunityTableMap';
     ...
     */
    protected int|null $vol_id = null;
     ...
     */
    protected int|null $vol_order = null;
     ...
     */
    protected string|null $vol_active = null;

    /**
     * The value for the vol_name field.
     */
    protected string|null $vol_name = null;

    /**
     * The value for the vol_description field.
     */
    protected string|null $vol_description = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\VolunteerOpportunity object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : VolunteerOpportunityTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->vol_id = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : VolunteerOpportunityTableMap::translateFieldName('Order', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->vol_order = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : VolunteerOpportunityTableMap::translateFieldName('Active', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->vol_active = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : VolunteerOpportunityTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->vol_name = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : VolunteerOpportunityTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->vol_description = $columnValue !== null ? (string)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\VolunteerOpportunity object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildVolunteerOpportunityQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
    ...
    {
        $tableMap = VolunteerOpportunityTableMap::getTableMap();
        $criteria = new Criteria(VolunteerOpportunityTableMap::DATABASE_NAME);

        if ($this->isColumnModified(VolunteerOpportunityTableMap::COL_VOL_ID)) {
            $criteria->setUpdateValue($tableMap->getColumn('vol_ID'), $this->vol_id);
        }
        if ($this->isColumnModified(VolunteerOpportunityTableMap::COL_VOL_ORDER)) {
            $criteria->setUpdateValue($tableMap->getColumn('vol_Order'), $this->vol_order);
        }
        if ($this->isColumnModified(VolunteerOpportunityTableMap::COL_VOL_ACTIVE)) {
            $criteria->setUpdateValue($tableMap->getColumn('vol_Active'), $this->vol_active);
        }
        if ($this->isColumnModified(VolunteerOpportunityTableMap::COL_VOL_NAME)) {
            $criteria->setUpdateValue($tableMap->getColumn('vol_Name'), $this->vol_name);
        }
        if ($this->isColumnModified(VolunteerOpportunityTableMap::COL_VOL_DESCRIPTION)) {
            $criteria->setUpdateValue($tableMap->getColumn('vol_Description'), $this->vol_description);
        }
    ...
    {
        $tableMap = VolunteerOpportunityTableMap::getTableMap();
        $query = ChildVolunteerOpportunityQuery::create();
        $vol_IDColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('vol_ID'));
        $query->addAnd($vol_IDColumn, $this->vol_id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\VolunteerOpportunity (or compatible) type.
