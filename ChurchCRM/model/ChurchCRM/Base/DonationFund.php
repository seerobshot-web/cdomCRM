...
abstract class DonationFund implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\DonationFundTableMap';
     ...
     */
    protected int|null $fun_id = null;
     ...
     */
    protected string|null $fun_active = null;

    /**
     * The value for the fun_name field.
     */
    protected string|null $fun_name = null;

    /**
     * The value for the fun_description field.
     */
    protected string|null $fun_description = null;
     ...
     */
    protected int|null $fun_order = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\DonationFund object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : DonationFundTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->fun_id = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : DonationFundTableMap::translateFieldName('Active', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->fun_active = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : DonationFundTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->fun_name = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : DonationFundTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->fun_description = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : DonationFundTableMap::translateFieldName('Order', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->fun_order = $columnValue !== null ? (int)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\DonationFund object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildDonationFundQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
    ...
    {
        $tableMap = DonationFundTableMap::getTableMap();
        $criteria = new Criteria(DonationFundTableMap::DATABASE_NAME);

        if ($this->isColumnModified(DonationFundTableMap::COL_FUN_ID)) {
            $criteria->setUpdateValue($tableMap->getColumn('fun_ID'), $this->fun_id);
        }
        if ($this->isColumnModified(DonationFundTableMap::COL_FUN_ACTIVE)) {
            $criteria->setUpdateValue($tableMap->getColumn('fun_Active'), $this->fun_active);
        }
        if ($this->isColumnModified(DonationFundTableMap::COL_FUN_NAME)) {
            $criteria->setUpdateValue($tableMap->getColumn('fun_Name'), $this->fun_name);
        }
        if ($this->isColumnModified(DonationFundTableMap::COL_FUN_DESCRIPTION)) {
            $criteria->setUpdateValue($tableMap->getColumn('fun_Description'), $this->fun_description);
        }
        if ($this->isColumnModified(DonationFundTableMap::COL_FUN_ORDER)) {
            $criteria->setUpdateValue($tableMap->getColumn('fun_Order'), $this->fun_order);
        }
    ...
    {
        $tableMap = DonationFundTableMap::getTableMap();
        $query = ChildDonationFundQuery::create();
        $fun_IDColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('fun_ID'));
        $query->addAnd($fun_IDColumn, $this->fun_id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\DonationFund (or compatible) type.
    ...
    /**
     * Gets donationfund_fun objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this DonationFund is new, it will return
    ...
    /**
     * Sets a collection of donationfund_fun objects related by a one-to-many relationship
    ...
    /**
     * Returns the number of related donationfund_fun objects.
     *
     * @param \Propel\Runtime\ActiveQuery\Criteria|null $criteria
     * @param bool $distinct
     * @param \Propel\Runtime\Connection\ConnectionInterface|null $con
     *
     * @return int Count of related donationfund_fun objects.
     */
     ...
     * Otherwise if this donationfund_fun is new, it will return
     * an empty collection; or if this donationfund_fun has previously
