...
abstract class Token implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\TokenTableMap';
     ...
     */
    protected DateTimeInterface|null $valid_until_date = null;
     ...
     */
    protected int|null $reference_id = null;
     ...
     * Note: this column has a database default value of: 0
     */
    protected int|null $remaininguses = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\Token object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : TokenTableMap::translateFieldName('Token', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->token = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : TokenTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->type = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : TokenTableMap::translateFieldName('ValidUntilDate', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            if ($columnValue === '0000-00-00') {
                $columnValue = null;
            }
            $this->valid_until_date = ($columnValue !== null) ? PropelDateTime::newInstance($columnValue, null, '\DateTime') : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : TokenTableMap::translateFieldName('ReferenceId', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->reference_id = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : TokenTableMap::translateFieldName('RemainingUses', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->remaininguses = $columnValue !== null ? (int)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\Token object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildTokenQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'valid_until_date':
                        $stmt->bindValue($identifier, $this->valid_until_date ? $this->valid_until_date->format('Y-m-d') : null, PDO::PARAM_STR);
    ...
    public function getByPosition(int $pos)
    {
        return match ($pos) {
            0 => $this->getToken(),
            1 => $this->getType(),
            2 => $this->getValidUntilDate(),
            3 => $this->getReferenceId(),
            4 => $this->getRemainingUses(),
            default => null
        };
    }
    ...
    {
        $tableMap = TokenTableMap::getTableMap();
        $criteria = new Criteria(TokenTableMap::DATABASE_NAME);

        if ($this->isColumnModified(TokenTableMap::COL_TOKEN)) {
            $criteria->setUpdateValue($tableMap->getColumn('token'), $this->token);
        }
        if ($this->isColumnModified(TokenTableMap::COL_TYPE)) {
            $criteria->setUpdateValue($tableMap->getColumn('type'), $this->type);
        }
        if ($this->isColumnModified(TokenTableMap::COL_VALID_UNTIL_DATE)) {
            $criteria->setUpdateValue($tableMap->getColumn('valid_until_date'), $this->valid_until_date);
        }
        if ($this->isColumnModified(TokenTableMap::COL_REFERENCE_ID)) {
            $criteria->setUpdateValue($tableMap->getColumn('reference_id'), $this->reference_id);
        }
        if ($this->isColumnModified(TokenTableMap::COL_REMAININGUSES)) {
            $criteria->setUpdateValue($tableMap->getColumn('remainingUses'), $this->remaininguses);
        }
    ...
    {
        $tableMap = TokenTableMap::getTableMap();
        $query = ChildTokenQuery::create();
        $tokenColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('token'));
        $query->addAnd($tokenColumn, $this->token);
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\Token (or compatible) type.
