...
abstract class Config implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\ConfigTableMap';
     ...
     */
    protected string|null $cfg_name = null;

    /**
     * The value for the cfg_value field.
     */
    protected string|null $cfg_value = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\Config object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : ConfigTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->cfg_name = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : ConfigTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->cfg_value = $columnValue !== null ? (string)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\Config object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildConfigQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
    ...
    {
        $tableMap = ConfigTableMap::getTableMap();
        $criteria = new Criteria(ConfigTableMap::DATABASE_NAME);

        if ($this->isColumnModified(ConfigTableMap::COL_CFG_NAME)) {
            $criteria->setUpdateValue($tableMap->getColumn('cfg_name'), $this->cfg_name);
        }
        if ($this->isColumnModified(ConfigTableMap::COL_CFG_VALUE)) {
            $criteria->setUpdateValue($tableMap->getColumn('cfg_value'), $this->cfg_value);
        }
    ...
    {
        $tableMap = ConfigTableMap::getTableMap();
        $query = ChildConfigQuery::create();
        $cfg_nameColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('cfg_name'));
        $query->addAnd($cfg_nameColumn, $this->cfg_name);
    ...
    /**
     * Generic method to set the primary key (cfg_name column).
     *
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\Config (or compatible) type.
