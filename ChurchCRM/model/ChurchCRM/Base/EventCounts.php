...
abstract class EventCounts implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\EventCountsTableMap';
     ...
     */
    protected int|null $evtcnt_eventid = null;
     ...
     * Note: this column has a database default value of: 0
     */
    protected int|null $evtcnt_countid = null;

    /**
     * The value for the evtcnt_countname field.
     */
    protected string|null $evtcnt_countname = null;

    /**
     * The value for the evtcnt_countcount field.
     */
    protected int|null $evtcnt_countcount = null;

    /**
     * The value for the evtcnt_notes field.
     */
    protected string|null $evtcnt_notes = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\EventCounts object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : EventCountsTableMap::translateFieldName('EvtcntEventid', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->evtcnt_eventid = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : EventCountsTableMap::translateFieldName('EvtcntCountid', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->evtcnt_countid = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : EventCountsTableMap::translateFieldName('EvtcntCountname', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->evtcnt_countname = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : EventCountsTableMap::translateFieldName('EvtcntCountcount', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->evtcnt_countcount = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : EventCountsTableMap::translateFieldName('EvtcntNotes', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->evtcnt_notes = $columnValue !== null ? (string)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\EventCounts object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildEventCountsQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
    ...
    {
        return match ($pos) {
            0 => $this->getEvtcntEventid(),
            1 => $this->getEvtcntCountid(),
            2 => $this->getEvtcntCountname(),
            3 => $this->getEvtcntCountcount(),
            4 => $this->getEvtcntNotes(),
            default => null
        };
    }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(EventCountsTableMap::COL_EVTCNT_EVENTID)) {
            $criteria->setUpdateValue($tableMap->getColumn('evtcnt_eventid'), $this->evtcnt_eventid);
        }
        if ($this->isColumnModified(EventCountsTableMap::COL_EVTCNT_COUNTID)) {
            $criteria->setUpdateValue($tableMap->getColumn('evtcnt_countid'), $this->evtcnt_countid);
        }
        if ($this->isColumnModified(EventCountsTableMap::COL_EVTCNT_COUNTNAME)) {
            $criteria->setUpdateValue($tableMap->getColumn('evtcnt_countname'), $this->evtcnt_countname);
        }
        if ($this->isColumnModified(EventCountsTableMap::COL_EVTCNT_COUNTCOUNT)) {
            $criteria->setUpdateValue($tableMap->getColumn('evtcnt_countcount'), $this->evtcnt_countcount);
        }
        if ($this->isColumnModified(EventCountsTableMap::COL_EVTCNT_NOTES)) {
            $criteria->setUpdateValue($tableMap->getColumn('evtcnt_notes'), $this->evtcnt_notes);
        }
    ...
    {
        $tableMap = EventCountsTableMap::getTableMap();
        $query = ChildEventCountsQuery::create();
        $evtcnt_eventidColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('evtcnt_eventid'));
        $query->addAnd($evtcnt_eventidColumn, $this->evtcnt_eventid);
        $evtcnt_countidColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('evtcnt_countid'));
        $query->addAnd($evtcnt_countidColumn, $this->evtcnt_countid);
    ...
    {
        $pkIsValid = $this->getEvtcntEventid() !== null &&
                     $this->getEvtcntCountid() !== null;
    ...
    {
        return ($this->getEvtcntEventid() === null) && ($this->getEvtcntCountid() === null);
    }
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\EventCounts (or compatible) type.
