...
abstract class Deposit implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\ChurchCRM\\model\\ChurchCRM\\Map\\DepositTableMap';
     ...
     */
    protected int|null $dep_id = null;

    /**
     * The value for the dep_date field.
     */
    protected DateTimeInterface|null $dep_date = null;

    /**
     * The value for the dep_comment field.
     */
    protected string|null $dep_comment = null;

    /**
     * The value for the dep_enteredby field.
     */
    protected int|null $dep_enteredby = null;
     ...
     */
    protected bool|null $dep_closed = null;
     ...
     */
    protected string|null $dep_type = null;
    ...
    /**
     * Initializes internal state of ChurchCRM\model\ChurchCRM\Base\Deposit object.
     *
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $rowIndex = $useNumericIndex ? $startcol + 0 : DepositTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->dep_id = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 1 : DepositTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            if ($columnValue === '0000-00-00') {
                $columnValue = null;
            }
            $this->dep_date = ($columnValue !== null) ? PropelDateTime::newInstance($columnValue, null, '\DateTime') : null;

            $rowIndex = $useNumericIndex ? $startcol + 2 : DepositTableMap::translateFieldName('Comment', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->dep_comment = $columnValue !== null ? (string)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 3 : DepositTableMap::translateFieldName('Enteredby', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->dep_enteredby = $columnValue !== null ? (int)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 4 : DepositTableMap::translateFieldName('Closed', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->dep_closed = $columnValue !== null ? (bool)$columnValue : null;

            $rowIndex = $useNumericIndex ? $startcol + 5 : DepositTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType);
            $columnValue = $row[$rowIndex];
            $this->dep_type = $columnValue !== null ? (string)$columnValue : null;
            ...
            throw new PropelException('Error populating \ChurchCRM\model\ChurchCRM\Deposit object', 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildDepositQuery::create(null, $this->buildPkeyCriteria())->fetch($con);
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'dep_ID':
                        ...
                        break;
                    case 'dep_Date':
                        $stmt->bindValue($identifier, $this->dep_date ? $this->dep_date->format('Y-m-d') : null, PDO::PARAM_STR);
    ...
    {
        $tableMap = DepositTableMap::getTableMap();
        $criteria = new Criteria(DepositTableMap::DATABASE_NAME);

        if ($this->isColumnModified(DepositTableMap::COL_DEP_ID)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_ID'), $this->dep_id);
        }
        if ($this->isColumnModified(DepositTableMap::COL_DEP_DATE)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_Date'), $this->dep_date);
        }
        if ($this->isColumnModified(DepositTableMap::COL_DEP_COMMENT)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_Comment'), $this->dep_comment);
        }
        if ($this->isColumnModified(DepositTableMap::COL_DEP_ENTEREDBY)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_EnteredBy'), $this->dep_enteredby);
        }
        if ($this->isColumnModified(DepositTableMap::COL_DEP_CLOSED)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_Closed'), $this->dep_closed);
        }
        if ($this->isColumnModified(DepositTableMap::COL_DEP_TYPE)) {
            $criteria->setUpdateValue($tableMap->getColumn('dep_Type'), $this->dep_type);
        }
    ...
    {
        $tableMap = DepositTableMap::getTableMap();
        $query = ChildDepositQuery::create();
        $dep_IDColumn = new LocalColumnExpression($query, $tableMap->getName(), $tableMap->getColumn('dep_ID'));
        $query->addAnd($dep_IDColumn, $this->dep_id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \ChurchCRM\model\ChurchCRM\Deposit (or compatible) type.
    ...
    /**
     * Gets deposit_dep objects which contain a foreign key that references this object.
     *
    ...
    /**
     * Sets a collection of deposit_dep objects related by a one-to-many relationship
    ...
    /**
     * Returns the number of related deposit_dep objects.
     *
     * @param \Propel\Runtime\ActiveQuery\Criteria|null $criteria
     * @param bool $distinct
     * @param \Propel\Runtime\Connection\ConnectionInterface|null $con
     *
     * @return int Count of related deposit_dep objects.
     */
     ...
     * Otherwise if this deposit_dep is new, it will return
     * an empty collection; or if this deposit_dep has previously
